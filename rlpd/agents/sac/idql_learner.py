...
from rlpd.networks import DiffusionMLP, DDPM, FourierFeatures, cosine_beta_schedule, ddpm_sampler, ddim_sampler, ddpm_train_sampler, DiffusionMLPResNet, get_weight_decay_mask, vp_beta_schedule
...
class IDQLLearner(Agent):
    ...
    def update_value(self, batch: DatasetDict) -> Tuple[Agent, Dict[str, float]]:
        ...
        def value_loss_fn(value_params) -> Tuple[jnp.ndarray, Dict[str, float]]:
            v = self.value.apply_fn({"params": value_params}, batch["observations"], True,
                rngs={"dropout": key})
